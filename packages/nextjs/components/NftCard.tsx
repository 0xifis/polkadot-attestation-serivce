...
const NFTCard = ({ nft }: { nft: any }) => {
  return (
    <CardContainer>
      <CardBody className="group/card relative h-auto w-auto rounded-xl border border-black/[0.1] bg-primary-500 p-6 dark:border-white/[0.2] dark:bg-black dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1]">
        ...
        <div className="mt-6 flex items-center justify-between">
          <CardItem translateZ={20} as="button" className="rounded-xl px-4 py-2 text-xs font-normal text-white">
            <Link href={`/nft/${nft.collectionAddress}/${nft.collectionTokenId}`}>View Attestations →</Link>
          </CardItem>
          <CardItem
            translateZ={20}
            as="button"
